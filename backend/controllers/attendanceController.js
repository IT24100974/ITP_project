...
const createSession = async (req, res, next) => {
  try {
    const { classId, date, startTime, endTime, topic, notes, isExtraSession } = req.body;
...
const updateSession = async (req, res, next) => {
  try {
    ...
    const { topic, notes, status, startTime, endTime } = req.body;
...
const cancelSession = async (req, res, next) => {
  try {
    ...
    // Remove attendance records for cancelled session
    await Attendance.deleteMany({ session: session._id });

    res.status(200).json({
      success: true,
      message: 'Session cancelled and attendance records removed'
    });
  } catch (error) {
    next(error);
  }
